...
for n in f:    
  n = n .rstrip ()
  url = 'https://api.bitbucket.org/1.0/repositories/' + n + '/' + collName
  ...
  while True:
    try: 
      ...
      if ('events' in jsonDict):
        for el in jsonDict ['events']:
          v. append (el)
  ...
  if len (v) > 0:
    ...
    if (size < 16777216/3):
      coll1.insert ( { 'full_name': n, 'values': v } )
    else:
      s = size;
      n = 3*s/16777216
      i = 0
      for ch in chunks (v, n):
        coll1.insert ( { 'chunk': i, 'full_name': n, 'values': ch } )
        i = i + 1 
